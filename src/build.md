...
> **注意:** `build-jar` はプロジェクト自体をコンパイルし、コンパイル済みの JAR ファイルを `artifact` ディレクトリに配置します。`build-jar` を実行する前に `build` や `build-classes` を実行する必要はありません。
...
> **注意:** `build-fatjar` はプロジェクト自体をコンパイルし、コンパイル済みの JAR ファイルを `artifact` ディレクトリに配置します。`build-fatjar` を実行する前に `build` や `build-classes` を実行する必要はありません。
